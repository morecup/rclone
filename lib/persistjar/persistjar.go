...
func (j *PersistJar) cookies(u *url.URL, now time.Time) (cookies []*http.Cookie) {
...
func (j *PersistJar) setCookies(u *url.URL, cookies []*http.Cookie, now time.Time) {
	if len(cookies) == 0 {
		return
	}
	if u.Scheme != "http" && u.Scheme != "https" {
...
func (j *PersistJar) newEntry(c *http.Cookie, now time.Time, defPath, host string) (e entry, remove bool, err error) {
	e.Name = c.Name
...
func (j *PersistJar) domainAndType(host, domain string) (string, bool, error) {
	if domain == "" {
...
func (j *PersistJar) Save() {
	...
	j.m.Set(j.cookieKey, string(data))
}
func (j *PersistJar) Load() {
	if j.m == nil {
		return
	}
	value, ok := j.m.Get(j.cookieKey)
	if ok {
		err := json.Unmarshal([]byte(value), &j.entries)
		if err != nil {
