...
The `Sites.Read.All` permission is required if you need to [search SharePoint sites when configuring the remote](https://github.com/morecup/rclone/pull/5883). However, if that permission is not assigned, you need to exclude `Sites.Read.All` from your access scopes or set `disable_site_permission` option to true in the advanced options.
...
Note: If you have a special region, you may need a different host in step 4 and 5. Here are [some hints](https://github.com/morecup/rclone/blob/bc23bf11db1c78c6ebbf8ea538fbebf7058b4176/backend/onedrive/onedrive.go#L86).
...
[#2707](https://github.com/morecup/rclone/issues/2707) for more info.
...
Shared with me files is not supported by rclone [currently](https://github.com/morecup/rclone/issues/4062), but there is a workaround:
